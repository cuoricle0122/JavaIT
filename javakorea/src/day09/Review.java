...
public class Review {
	public static void main(String[] args) {
		...
		// 변수 : 저장공간 (수정, 보완)
		// 자료형 : 변수에게 줄 공간의 크기 및 형태 (형태에 조금 더 주목)
		String 문자열변수 = "문자열저장";
		int 정수형변수 = 314;
		double 실수형변수 = 3.14;
		boolean 불린형변수 = true;		// true | false
		
		System.out.println(문자열변수);
		System.out.println(정수형변수);
		System.out.println(실수형변수);
		System.out.println(불린형변수);
		...
		문자열변수 = sc.nextLine();							// 문장을 입력(띄어쓰기 가능, 띄어쓰기 가능버그가 있어서 next, nextInt와 혼용 금지)
		정수형변수 = Integer.parseInt(sc.nextLine());		// 정수를 입력(nextInt 대체용)						→ 가능한 이렇게 쓰는게 더 좋음
		실수형변수 = Double.parseDouble(sc.nextLine());		// 입력받은 문자열을 실수로 변환(nextDouble 대체용)		→ 가능한 이렇게 쓰는게 더 좋음
...
class ReviewTest extends TheOne implements TheTwo{
	private int num[] = {10,20,30,40,50};				// int num → 멤버변수
	
	public void go() {									// 메서드 public
		for(int i=0;i<5;i++) {							// int i → 지역변수
			System.out.println(num[i]);					// 지역변수 i는 중괄호가 끝나면 없어짐
		}
	}
}
...
class TheOne{
	private String str;
	public TheOne() {					// 클래스 이름과 같고 소괄호가 있으면 '생성자', 자료형이 없음 // 생성자 : 객체화하는 순간에 사용될 메서드
		System.out.println("객체화 성공");
	}
	public String getStr() {			// 메서드의 자료형 : return의 자료형
		return str;						// return : 함수를 종료시키면서 해당 값을 사용한 곳에 전달
	}
	public void setStr(String str) {	// ( )안의 값 : 매개변수, 사용하는 곳에서 전달해줘야하는 값을 지정하는 변수들
		this.str = str;					// this. : 이 클래스의, 멤버에 있는 변수나 메서드를 사용하기 위해 앞에 붙여줌
	}
}

interface TheTwo{
	public void go();		// 구현X, 인터페이스의 메서드는 상속받은 곳에서 {}구현해줘야함
}
